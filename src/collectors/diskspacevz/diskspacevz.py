...
class DiskSpaceCollectorVZ(diamond.collector.Collector):

    def get_default_config_help(self):
        config_help = super(DiskSpaceCollectorVZ, self).get_default_config_help()
        config_help.update({
    ...
    def get_default_config(self):
        ...
        """
        config = super(DiskSpaceCollectorVZ, self).get_default_config()
        config.update({
            # Enabled by default
            'enabled': 'True',
            'path': 'diskspace',
    ...
    def __init__(self, config, handlers):
        super(DiskSpaceCollectorVZ, self).__init__(config, handlers)
        ...
        if os.access('/proc/mounts', os.R_OK):
            ...
            for line in file:
                ...
                if '/' in device and mount_point.startswith('/'):
                    ...
                    minor = 0
                    try:
			# os method crashes in openvz-containers, seems not necessary
                        minor += 1
                    except OSError:
